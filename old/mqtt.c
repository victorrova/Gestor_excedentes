...
enum MQTTErrors mqtt_init(struct mqtt_client *client,
               ...
               uint8_t *sendbuf, ssize_t sendbufsz,
               uint8_t *recvbuf, ssize_t recvbufsz,
               void (*publish_response_callback)(void** state,struct mqtt_response_publish *publish))
{
    ...
    /* initialize mutex */
    _mutex_init(&client->mutex);
    xSemaphoreTake(&client->mutex,portMAX_DELAY); /* unlocked during CONNECT */
...
#define MQTT_CLIENT_TRY_PACK(tmp, msg, client, pack_call, release)  \
    if (client->error < 0) {                                        \
        if (release) xSemaphoreGive(&client->mutex);         \
        return client->error;                                       \
    }                                                               \
    tmp = pack_call;                                                \
    if (tmp < 0) {                                                  \
    ...
    msg = mqtt_mq_register(&client->mq, (ssize_t)tmp);                       \
   ...
   xSemaphoreTake(&client->mutex,portMAX_DELAY);
    ...
    for(; i < len; ++i) {
        ...
        if (msg->state == MQTT_QUEUED_UNSENT) {
            ...
            /* check for timeout */
            if (esp_time_s() > msg->time_sent + client->response_timeout) {
                resend = 1;
        ...
        client->time_of_last_send = esp_time_s();
    ...
    /* check for keep-alive */
    {
        ...
        if (esp_time_s() > keep_alive_timeout) {
...
ssize_t __mqtt_recv(struct mqtt_client *client)
{
    ...
    while(mqtt_recv_ret == MQTT_OK) {
        ...
        /* attempt to parse */
        consumed = mqtt_unpack_response(&response, client->recv_buffer.mem_start, (ssize_t) (client->recv_buffer.curr - client->recv_buffer.mem_start));
        ...
        */
        switch (response.fixed_header.control_type) {
            case MQTT_CONTROL_CONNACK:
                ...
                client->typical_response_time = (float) (esp_time_s() - msg->time_sent);
            ...
            case MQTT_CONTROL_PUBACK:
                ...
                client->typical_response_time = 0.875f * (client->typical_response_time) + 0.125f * (float) (esp_time_s() - msg->time_sent);
                break;
            case MQTT_CONTROL_PUBREC:
                /* check if this is a duplicate */
        ...
        }
        {
          ...
          ssize_t n = (ssize_t) (client->recv_buffer.curr - client->recv_buffer.mem_start - consumed);
          memmove(dest, src, n);
...
ssize_t mqtt_unpack_fixed_header(struct mqtt_response *response, const uint8_t *buf, ssize_t bufsz) {
...
ssize_t mqtt_pack_fixed_header(uint8_t *buf, ssize_t bufsz, const struct mqtt_fixed_header *fixed_header) {
    const uint8_t *start = buf;
    ssize_t errcode;
...
/* CONNECT */
ssize_t mqtt_pack_connection_request(uint8_t* buf, ssize_t bufsz,
                                     const char* client_id,
                                     const char* will_topic,
                                     const void* will_message,
                                     ssize_t will_message_size,
...
/* DISCONNECT */
ssize_t mqtt_pack_disconnect(uint8_t *buf, ssize_t bufsz) {
    struct mqtt_fixed_header fixed_header;
...
/* PING */
ssize_t mqtt_pack_ping_request(uint8_t *buf, ssize_t bufsz) {
    struct mqtt_fixed_header fixed_header;
...
/* PUBLISH */
ssize_t mqtt_pack_publish_request(uint8_t *buf, ssize_t bufsz,
                                  const char* topic_name,
                                  uint16_t packet_id,
                                  const void* application_message,
                                  ssize_t application_message_size,
...
/* PUBXXX */
ssize_t mqtt_pack_pubxxx_request(uint8_t *buf, ssize_t bufsz, 
...
ssize_t mqtt_unpack_suback_response (struct mqtt_response *mqtt_response, const uint8_t *buf) {
    ...
    /* unpack return codes */
    mqtt_response->decoded.suback.num_return_codes = (ssize_t) remaining_length;
...
/* SUBSCRIBE */
ssize_t mqtt_pack_subscribe_request(uint8_t *buf, ssize_t bufsz, unsigned int packet_id, ...) {
    va_list args;
    const uint8_t *const start = buf;
    ssize_t rv;
    struct mqtt_fixed_header fixed_header;
...
/* UNSUBSCRIBE */
ssize_t mqtt_pack_unsubscribe_request(uint8_t *buf, ssize_t bufsz, unsigned int packet_id, ...) {
    va_list args;
    const uint8_t *const start = buf;
    ssize_t rv;
    struct mqtt_fixed_header fixed_header;
...
/* MESSAGE QUEUE */
void mqtt_mq_init(struct mqtt_message_queue *mq, void *buf, ssize_t bufsz) 
{  
    mq->mem_start = buf;
...
struct mqtt_queued_message* mqtt_mq_register(struct mqtt_message_queue *mq, ssize_t nbytes)
{
    ...
    mq->curr += nbytes;
    mq->curr_sz = (ssize_t) (mqtt_mq_currsz(mq));
...
void mqtt_mq_clean(struct mqtt_message_queue *mq) {
    ...
    /* move buffered data */
    {
        ssize_t n = (ssize_t) (mq->curr - new_head->start);
        ssize_t removing = (ssize_t) (new_head->start - (uint8_t*) mq->mem_start);
        ...
        /* move queue */
        {
            ...
            memmove(mqtt_mq_get(mq, new_tail_idx), mq->queue_tail, sizeof(struct mqtt_queued_message) * (ssize_t) ((new_tail_idx + 1)));
...
/* RESPONSE UNPACKING */
ssize_t mqtt_unpack_response(struct mqtt_response* response, const uint8_t *buf, ssize_t bufsz) {
    const uint8_t *const start = buf;
